...
def _summarize_old_exchanges(messages: list[dict], keep_recent: int = 60) -> list[dict]:
    ...
    if not topics:
        return recent
    summary_text = " | ".join(topics[-40:])
    return [{"role": "system", "content": f"[Earlier conversation summary (key points the user discussed): {summary_text}]"}, *recent]
