...
from config import (
    STRIKES_FOR_BAN, STRIKE_TIMEOUT_SECONDS, AUTOMOD_TIMEOUT_SECONDS,
    FILTER_COOLDOWN_SECONDS, BLACKLISTED_WORDS,
...
class Moderation(commands.Cog, name="Moderation"):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        try:
            ...
            dm_embed.set_footer(text="Botdi Auto-Mod • Repeated violations lead to a strike")
    ...
    @commands.hybrid_command(name="strike", description="Issue a strike to a user")
    ...
    @commands.hybrid_command(name="warn", description="Send a warning DM to a user (no strike)")
    @commands.has_permissions(moderate_members=True)
    @discord.app_commands.describe(user="User ID or @mention", reason="Reason for the warning")
    ...
    @commands.hybrid_command(name="slowmode", description="Set channel slowmode (0 to disable)")
    @commands.has_permissions(manage_channels=True)
    @discord.app_commands.describe(seconds="Slowmode delay in seconds (0-21600, 0 to disable)")
    ...
    @commands.hybrid_command(name="lock", description="Lock a channel so no one can send messages")
    @commands.has_permissions(manage_channels=True)
    ...
    async def lock_cmd(self, ctx: commands.Context, channel: discord.TextChannel | None = None) -> None:
        ...
        await ch.send(embed=discord.Embed(title="🔒 Channel Locked", description="This channel has been locked by a moderator.", color=0xE74C3C))
        if channel and channel != ctx.channel:
            await ctx.send(f"✅ {ch.mention} has been locked.", delete_after=10)
        await log_action(self.bot, "🔒 Channel Locked", f"**Channel:** {ch.mention}\n**Moderator:** {ctx.author.mention}")
    ...
    async def unlock_cmd(self, ctx: commands.Context, channel: discord.TextChannel | None = None) -> None:
        ...
        await ch.set_permissions(ctx.guild.default_role, overwrite=overwrite)
        await ch.send(embed=discord.Embed(title="🔓 Channel Unlocked", description="This channel has been unlocked.", color=0x2ECC71))
        if channel and channel != ctx.channel:
            await ctx.send(f"✅ {ch.mention} has been unlocked.", delete_after=10)
        await log_action(self.bot, "🔓 Channel Unlocked", f"**Channel:** {ch.mention}\n**Moderator:** {ctx.author.mention}", color=0x2ECC71)
